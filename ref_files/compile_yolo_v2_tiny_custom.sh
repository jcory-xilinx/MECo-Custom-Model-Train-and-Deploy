...
if [[ $mode == $STATIC ]]; then
  ...
  qmname=""
  mdir="${mroot}/${mname}"
  if [[ ${repo_name} == "ml-frontend" && -z ${start_time_step} ]]; then
    echo "Checking out retrain models"
    echo "Retrain mode models not found"
    exit 1
  fi
fi
